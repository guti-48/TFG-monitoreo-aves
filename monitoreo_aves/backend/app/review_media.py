...
def get_clean_review_audio_path(
    ...
    with _AUDIO_LOCK:
        ...
        reference_peak = float(np.percentile(np.abs(clean), 99.9))
        if reference_peak > np.finfo(float).eps:
            gain = min(12.0, 0.82 / reference_peak)
            clean = clean * gain
        clean = np.clip(clean, -0.98, 0.98).astype(np.float32)
